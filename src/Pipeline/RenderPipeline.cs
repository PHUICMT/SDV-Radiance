    ...
    internal sealed partial class RenderPipeline : IDisposable
    {
        ...
        private Texture2D FallDistanceFarTexture()
        {
            if (_fallDistanceFarTexture == null || _fallDistanceFarTexture.IsDisposed)
            {
                ...
                _fallDistanceFarTexture.SetData([new Color(255, 255, 0, 255)]);
            }
        ...
        // The composed water flags for the mask currently on screen. A copy taken when a
        // rebuild lands, because _waterTileFlags belongs to whatever rebuild is
        // running next and a worker thread is writing it.
        // The field this describes lives in ScreenState now; see RenderPipeline.Screens.cs.
        // Bumped every time _waterTilesInMask is refilled. The array is
        // reused in place, so nothing downstream can tell it changed by looking at it; the
        // summed-area table that answers "is there water near here" keys its cache off this.
        // The field this describes lives in ScreenState now; see RenderPipeline.Screens.cs.
        ...
        private int[]? _structureScrubTopScratch;        // per-column art-top scratch for the struct march scrub (16 entries)
        private int[]? _structureScrubBottomScratch;     // per-column art-bottom scratch for the struct march scrub (16 entries)
        ...
        // The whole-tilesheet pixel cache moved to SheetPixels, which the shadow pass shares: it
        // asks the same tilesheets this does and used to read and hold every one of them a second
        // time. Reading each 16×16 tile with its own texture.GetData is a separate GPU→CPU readback
        // (a pipeline flush ~1-3 ms EACH, regardless of the tiny size); walking into a fresh
        // forest/town screen touched 100+ new tiles in one gather → a ~300 ms main-thread stall.
        // Huge sheets (over SheetPixels.PixelCap) still fall back to per-region GetData.
        ...
        private readonly System.Collections.Generic.Dictionary<(Texture2D, Rectangle), Color[]> _tileArtCache = [];
        private ref GameLocation? _prewarmedLocation => ref _screen.PrewarmedLocation;   // last location whose tilesheets we bulk-read back
        ...
        /// <summary>When this screen last ran <see cref="Apply"/>, by screen id.
        ///
        /// <para>PER SCREEN, because <see cref="Apply"/> runs once per screen per frame. A single
        /// stamp measured the gap between CALLS, so in split screen the second screen saw the
        /// millisecond or two since the first screen's Apply rather than the time since its own
        /// last frame: clamped to the floor, every fade on that screen stepped at a tenth to a
        /// sixteenth of the first screen's speed. The fix for fades running too fast when the
        /// frame cap is lifted had quietly made the mirror of it on anyone playing in split
        /// screen, where a fade can take ten seconds to arrive.</para></summary>
        private static readonly Dictionary<int, long> _easeStampByScreen = [];
        private const double EaseSecondsFloor = 1.0 / 1000.0;
        private const double EaseSecondsCeiling = 3.0 / 60.0;
        private const double EaseSecondsDefault = 1.0 / 60.0;
        ...
        private static void StampEaseClock()
        {
            ...
            if (_easeStampByScreen.TryGetValue(screenId, out long previous))
            {
                double seconds = (now - previous) / (double)System.Diagnostics.Stopwatch.Frequency;
                _easeSeconds = Math.Clamp(seconds, EaseSecondsFloor, EaseSecondsCeiling);
            }
            else
                // A screen's first frame has nothing to measure, and the other screen's gap is not
                // an answer for it: one frame at sixty is the honest guess until it has its own.
                _easeSeconds = EaseSecondsDefault;
            _easeStampByScreen[screenId] = now;
        }

        /// <summary>Drop a departed screen's ease clock, so a screen id that comes back later
        /// starts over rather than measuring against whenever its predecessor last drew.</summary>
        private static void ForgetEaseClock(int screenId) => _easeStampByScreen.Remove(screenId);
        ...
        private static float Ease01(float value) => Determinism.Frozen ? 1f
            : value >= 0.999f ? 1f : Math.Min(1f, 1f - (1f - value) * EaseKeep(0.10f));
        ...
        private static float Ease0(float value) => Determinism.Frozen ? 0f
            : value <= 0.004f ? 0f : value * EaseKeep(0.10f);
        ...
        private static void Approach(ref float value, float target, float rate) =>
            value = Determinism.Settle(target + (value - target) * EaseKeep(rate), target);

        /// <summary>The same step for a direction or a colour. Written out at five call sites as a
        /// Lerp with a per-frame fraction, which is a machine rather than a unit: at an uncapped
        /// frame rate those five arrive between three and seven times too fast, and a fade that
        /// fast is the pop this family exists to prevent.</summary>
        private static void Approach(ref Vector2 value, Vector2 target, float rate) =>
            value = Determinism.Settle(target + (value - target) * EaseKeep(rate), target);

        private static void Approach(ref Vector3 value, Vector3 target, float rate) =>
            value = Determinism.Settle(target + (value - target) * EaseKeep(rate), target);

        /// <summary>One eased step toward a target, RETURNED rather than assigned, for the call
        /// sites that hand the result to Determinism.Settle themselves. Same clock as Approach.</summary>
        private static float EasedToward(float value, float target, float rate)
            => target + (value - target) * EaseKeep(rate);

        private static Vector3 EasedToward(Vector3 value, Vector3 target, float rate)
            => target + (value - target) * EaseKeep(rate);

        /// <summary>How fast a room's own eases travel: the exposure it settles at, the colour of
        /// its windows, its saturation, the daylight through the glass. Slow on purpose, because
        /// they are whole-room changes a player walks into. A per-frame fraction at sixty.</summary>
        private const float RoomEaseRate = 0.03f;

        /// <summary>How fast the sun shafts take a new strength, direction and colour. Every gate
        /// on them is a hard flip (rain starting, the toggle, a warp) and a hard flip on a
        /// whole-screen effect is a pop, so all three ease together over about a second.</summary>
        private const float ShaftEaseRate = 0.05f;

        /// <summary>One step of a ramp that climbs by a fixed amount per frame rather than by a
        /// fraction of what is left: the whole stack fading in when the mod is switched on. Same
        /// unit problem, same answer - the step is what it was at sixty frames a second.</summary>
        private static float RampTo1(float value, float perFrameAtSixty) =>
            Determinism.Settle(Math.Min(1f, value + perFrameAtSixty * (float)(_easeSeconds * 60.0)), 1f);

        /// <summary>The same ramp, down.</summary>
        private static float RampTo0(float value, float perFrameAtSixty) =>
            Determinism.Settle(Math.Max(0f, value - perFrameAtSixty * (float)(_easeSeconds * 60.0)), 0f);

        /// <summary>
        /// An overnight event that paints the whole screen itself, over the world.
        ///
        /// <para>The plane that drops the mystery boxes (QiPlaneEvent) and the sound in the night
        /// (SoundInTheNightEvent) are drawn into the world frame after the world, starting with an
        /// opaque rectangle the size of the screen: the night sky and the plane, or plain black.
        /// Under that rectangle the world is still the farmhouse the player went to sleep in, and
        /// every pass of ours still read the farmhouse (its lamps, walls, windows and the relief of
        /// its furniture) and laid what it found over the event's picture, which is the
        /// colourless impression of the house in the middle of the screen reported on Nexus
        /// (Mokayogi). While one of these is up the stack leaves the picture as the game draws it.
        /// Not <c>Game1.eventUp</c>: a cutscene is drawn in the world and keeps every effect. The
        /// other overnight events (the fairy, the witch, a birth, a question) show the real
        /// location or a dialogue box, and keep them too.</para>
        /// </summary>
        private static bool FarmEventPaintsTheScreen
            => Game1.farmEvent is StardewValley.Events.QiPlaneEvent or StardewValley.Events.SoundInTheNightEvent;

        /// <summary>How fast the stack leaves when such an event starts: four frames. It opens out
        /// of the sleep's black, so there is nothing on screen yet for the change to pop against,
        /// and at the master's own fade-in pace the house would show through for most of a second.</summary>
        private const float MasterFadeOutUnderFarmEvent = 0.25f;
        ...
        /// <summary>Hoed dirt in the location, watered or not, and where it lies: the answer to
        /// "is there anything for the watered-soil sheen to find here" without a walk in game.</summary>
        private static string DescribeHoedDirt()
        {
            GameLocation? location = Game1.currentLocation;
            if (location?.terrainFeatures == null)
                return "";
            int hoed = 0, watered = 0, minX = int.MaxValue, minY = int.MaxValue, maxX = int.MinValue, maxY = int.MinValue;
            foreach (var pair in location.terrainFeatures.Pairs)
            {
                if (pair.Value is not StardewValley.TerrainFeatures.HoeDirt dirt)
                    continue;
                hoed++;
                if (dirt.state.Value == 1) watered++;
                minX = Math.Min(minX, (int)pair.Key.X); maxX = Math.Max(maxX, (int)pair.Key.X);
                minY = Math.Min(minY, (int)pair.Key.Y); maxY = Math.Max(maxY, (int)pair.Key.Y);
            }
            return hoed == 0 ? "hoed dirt in location: none" : $"hoed dirt in location: {hoed} ({watered} watered) tiles x {minX}-{maxX} y {minY}-{maxY}";
        }
        ...
        // Eased twin of _hasWaterInMask: water scrolling into or out of
        // the mask window must not add or remove a whole pass in one frame.
        // The field this describes lives in ScreenState now; see RenderPipeline.Screens.cs.
        ...
        private void AccumulateBuildMilliseconds(int builderIndex, double milliseconds)
        {
            _buildMilliseconds[builderIndex] += milliseconds;
            if (milliseconds > _buildMaxMilliseconds[builderIndex]) _buildMaxMilliseconds[builderIndex] = milliseconds;
        }
        ...
        private bool TimedBuild(ModConfig config, int builderIndex, Func<bool> build)
        {
            long buildStart = FrameCost.Begin(_gridCostParts[builderIndex]);
            bool built = build();
            double milliseconds = FrameCost.End(_gridCostParts[builderIndex], buildStart);
            if (config.DebugLogging) AccumulateBuildMilliseconds(builderIndex, milliseconds);
            return built;
        }
        ...
        // Slot 4 was "rays" until the sun shafts moved inside the flood pass and their stage was
        // deleted with its shader, leaving a name nothing could ever print. The building shadow
        // needed one: it borrows the cloud shadow's effect (see RenderBuildingShadow) and was
        // registered under the cloud's name, so the report said the cloud pass ran twice a frame
        // when it ran once and the building shadow ran once. A pass nobody can name is a pass
        // nobody can measure.
        private static readonly string[] _stageNames = ["flood", "lighting", "water", "cloud", "building", "bloom", "fog", "grade", "tilt", "finish", "tail", "wet"];
        ...
        private readonly List<int> _stageNameIndices = [];
        ...
        private double ProbeGpuTime(SpriteBatch spriteBatch, RenderTarget2D target)
        {
            ...
            try
            {
                ...
                milliseconds = (System.Diagnostics.Stopwatch.GetTimestamp() - buildStart) * 1000.0 / System.Diagnostics.Stopwatch.Frequency;
                _gpuProbeTotalMilliseconds += milliseconds;
                if (milliseconds > _gpuProbeMaxMilliseconds) _gpuProbeMaxMilliseconds = milliseconds;
                _gpuProbeFrames++;
            }
        ...
        /// <summary>Armed by the dev harness only: two readbacks a frame, each a pipeline stall. A
        /// property rather than a field because the only writer is DevMenu.local.cs, which Release
        /// leaves out, and a field nothing assigns is a compiler warning in every Release build.</summary>
        internal static bool GainProbe { get; set; }
        ...
        /// </summary>
        /// <summary>Every setting that decides whether the world can look soft, on one line: the
        /// six blur reports of September 2026 each needed a round trip to learn one of these.</summary>
        private string DescribeSharpness()
        {
            ModConfig? configOfFrame = _lastConfig;
            float zoom = Game1.options?.zoomLevel ?? 1f;
            float uiScale = Game1.options?.uiScale ?? 1f;
            if (configOfFrame == null)
                return $"    sharpness: game zoom {zoom:P0}, ui scale {uiScale:P0} (no frame drawn yet)";
            string smoothArt = configOfFrame.SheetUpscaleEnabled ? configOfFrame.SheetUpscaleStyle.ToString() : "off";
            return $"    sharpness: game zoom {zoom:P0}, ui scale {uiScale:P0}, render scale {configOfFrame.RenderScale:0.00}"
                 + $"{(configOfFrame.RenderScaleAuto ? " (auto)" : "")}, perf preset {configOfFrame.ActivePerfPreset?.ToString() ?? "none"}, look {configOfFrame.ActivePreset},"
                 + $" smooth art {smoothArt}, relief {(configOfFrame.SpriteReliefEnabled ? "on" : "off")},"
                 + $" tilt-shift {(configOfFrame.TiltShiftEnabled ? "on" : "off")}, chromatic aberration {(configOfFrame.ChromaticAberrationEnabled ? "on" : "off")}"
                 + System.Environment.NewLine + SheetUpscaler.DescribeSoftSprites();
        }

        /// <summary>The relief pass, and which sheets it gave a bevel to last frame. A map's own
        /// tilesheet is meant to stay flat; one that shows up here is one the pass did not recognise,
        /// and every tile drawn from it wears an edge it should not.</summary>
        private string DescribeRelief()
        {
            if (_reliefEase <= FadeGone)
                return "    relief: off";
            var names = _bevelledSheetsLastFrame;
            string list = names.Count == 0 ? "none" : string.Join(", ", names.Take(10)) + (names.Count > 10 ? $" and {names.Count - 10} more" : "");
            string road = ReliefVertexRoad && _reliefVertexPath != null
                ? $"our own vertices ({_reliefVertexPath.SpritesLastFrame} sprites, {_reliefVertexPath.SheetsLastFrame} sheets, "
                  + $"{_reliefVertexPath.DrawsLastFrame} draws{(_reliefVertexPath.ClosedBecause.Length > 0 ? ", last refusal: " + _reliefVertexPath.ClosedBecause : "")})"
                : ReliefVertexRoad ? "our own vertices (nothing drawn yet)" : "the SpriteBatch";
            return $"    relief: presence {_reliefEase:F2}, road {road}, sheets bevelled last frame {names.Count}: {list}";
        }

        internal string DescribeStageState()
        {
            var report = new System.Text.StringBuilder();
            report.AppendLine($"frame {_frameWidth}x{_frameHeight}, effects computed at {_lastScaledWidth}x{_lastScaledHeight}");
            report.AppendLine($"ready: occluders={_isFloodOcclusionReady} shadows={_shadowsReady} waterOnScreen={_hasWaterInMask} "
                        + $"lights={_lightCount} meteredExposure={_meteredExposure:F3}");
            report.AppendLine($"    presence: flood={_fadeFlood:F3} water={_fadeWater:F3} cloud={_fadeCloud:F3} tilt={_fadeTilt:F3} building={_fadeBuildingShadow:F3} master={_masterFade:F3}");
            report.AppendLine($"    sun shafts: strength={_reportedShaftStrength:F3} dir=({_reportedShaftDirection.X:F2},{_reportedShaftDirection.Y:F2}) "
                        + $"(0 = a gate closed: sun source off, indoors, or no sun)");
            report.AppendLine($"    lamp shafts: strength={_reportedLampShaftStrength:F3} presence={_godRayAmount:F2} "
            ...
            report.AppendLine($"    GI: {(_cascades.Refused ? $"cascades refused by this device ({_cascades.RefusedReason})" : _cascades.LastReport)}");
            report.AppendLine($"    GI model in use: {(_lastCascadesInUse ? "cascades" : "flood, one texel per tile")}"
                        + (_lastConfig != null ? $" (setting: {_lastConfig.FloodGiModel})" : ""));
            report.AppendLine(DescribeSharpness());
            report.AppendLine("    " + DescribeAutoScale().Replace("\n", "\n    "));
            report.AppendLine(DescribeRelief());
            // Whether the guard that keeps a sheet from being handed this mod's linear sampler is
            // running: a report that says OFF or "not installed" next to a soft mailbox is the whole
            // diagnosis (see TextureUnitGuard).
            report.AppendLine($"    texture unit guard: {(TextureUnitGuard.Installed ? (TextureUnitGuard.Enabled ? "on" : "OFF (radiance_unitguard on)") : "NOT INSTALLED, see the startup log")}, {TextureUnitGuard.Rebinds} slot(s) rebound this session");
            report.AppendLine($"    sampler slots walked per draw call: {(TextureUnitGuard.SamplerSlotCap > 0 ? TextureUnitGuard.SamplerSlotCap.ToString() : "all")} of {TextureUnitGuard.SamplerSlots} the driver reports (radiance_samplerslots)");
            report.AppendLine("presence (0 = contributing nothing this frame):");
            report.AppendLine($"    water={_fadeWater:F2} flood={_fadeFlood:F2} lighting={_fadeLighting:F2} "
                        + $"cloud={_fadeCloud:F2} tilt={_fadeTilt:F2} godRays={_godRayAmount:F2}");
            report.AppendLine($"    fogDay={_fogDayAmount:F2} fogMist={_fogMistAmount:F2} cloudWeather={_cloudWeatherAmount:F2} "
                        + $"master={_masterFade:F2}");
            report.AppendLine($"    snowGlint={_snowGlintEase:F2} (season={LocalSky.Season} outdoors={Game1.currentLocation?.IsOutdoors} "
                        + $"snowing={Game1.currentLocation?.IsSnowingHere()} raining={Game1.currentLocation?.IsRainingHere()} night={NightFactorNow():F2})");
            report.AppendLine($"    wateredSoil={_wateredSparkleEase:F2} tilesDrawn={WateredSoilSparkle.TilesDrawnLastFrame} {DescribeHoedDirt()}");
            report.AppendLine($"    waterGlitter={_glitterPathEase:F2} sunAxis=({_glitterSunAxis.X:F2},{_glitterSunAxis.Y:F2}) (dial {_lastConfig?.WaterGlitterPath ?? 0f:F2})");
            report.AppendLine($"    wakeRings={_wakeRingsLive} live of {WakeRingSlots}, {_wakeRingMakers.Count} thing(s) in the water (dial {_lastConfig?.WaterWakeRings ?? 0f:F2}): {DescribeWakeRingMakers()}");
            report.AppendLine($"    fishSpot={DescribeFishSpot()} (dial {_lastConfig?.WaterFishSpotRings ?? 0f:F2})");
            report.AppendLine($"    wind={DescribeWaterWind()} (dial {_lastConfig?.WaterWind ?? 0f:F2})");
            report.AppendLine($"    current={DescribeWaterCurrent()}");
            if (_lastConfig != null)
                report.AppendLine($"    glass={DescribeGlass(_lastConfig)}");
            ...
            void Check(string name, Effect? effect) { if (effect == null) missing.Add(name); }
            ...
            report.AppendLine($"water mask: origin tile ({_lastWaterTileX},{_lastWaterTileY}) epoch {MaskEpoch} "
                        + $"rebuildInFlight={_pendingWaterMaskJob != null}");
            report.AppendLine($"labels: {(LabelStore.Instance == null ? "NOT LOADED (every water verdict falls back to the game's own data)" : $"loaded, v{LabelStore.Instance.Version}")}");
            ...
            if (LabelStore.Instance != null)
                report.AppendLine($"label sources: {LabelStore.Instance.DescribeSources()}");
            ...
            if (LabelStore.Instance is { ArtBoundLabelsRefusedForChangedArt: > 0 } guarded)
            {
                ...
                {
                    var claims = MapArtClaims.WhoPatches(refused.Key, _modDirectory, _monitor);
                    report.AppendLine($"  {refused.Key}: {refused.Value} tile(s)"
                                + (claims.Count == 0
            ...
            report.AppendLine("indoor light (windows, room level, what a lamp pool is worth):");
            report.AppendLine(DescribeIndoorLight());
            report.AppendLine(DescribeCameraKeyedCaches());
            report.AppendLine(DescribeSheetCache());
        ...
        private string DescribeSheetCache()
        {
            ...
            foreach (var sheetEntry in SheetPixels.Entries)
            {
                if (sheetEntry.Value == null)
                {
                    fellBack++;
                    if (offenders.Count < 6 && !sheetEntry.Key.IsDisposed)
                        offenders.Add($"{sheetEntry.Key.Width}x{sheetEntry.Key.Height}");
                    continue;
                }
                cached++;
                pixels += sheetEntry.Value.Length;
            }
        ...
        private float ProbeMean(SpriteBatch spriteBatch, Texture2D source, RenderTarget2D restore)
        {
            try
            {
                ...
                {
                    Color pixel = _gainProbePixels[i];
                    sum += (pixel.R * 0.299f + pixel.G * 0.587f + pixel.B * 0.114f) / 255f;
                }
        ...
        internal static bool HasLevelWater(GameLocation? location)
        {
            ...
            static bool NameContains(string text, string fragment) => text.IndexOf(fragment, StringComparison.OrdinalIgnoreCase) >= 0;
            return NameContains(locationName, "Mine") || NameContains(locationName, "Cave") || NameContains(locationName, "Volcano") || NameContains(locationName, "Dungeon")
                || NameContains(locationName, "Sewer") || NameContains(locationName, "Caldera") || NameContains(locationName, "Swamp") || NameContains(locationName, "BugLand")
                || NameContains(locationName, "Submarine") || NameContains(locationName, "Tunnel") || NameContains(locationName, "Grotto")
                || NameContains(locationName, "BathHouse") || NameContains(locationName, "Spa") || NameContains(locationName, "HotSpring") || NameContains(locationName, "Bath")
                || NameContains(typeName, "Mine") || NameContains(typeName, "Volcano") || NameContains(typeName, "Dungeon");
        }
        ...
        private readonly List<Action<SpriteBatch, Texture2D, RenderTarget2D, ModConfig>> _stages = [];
        ...
        // Not readonly: in split screen each camera keeps its own grid, and this holds
        // whichever screen's turn it currently is (see RenderPipeline.Screens.cs).
        // The field this describes lives in ScreenState now; see RenderPipeline.Screens.cs.
        ...
        /// <summary>The texture-sorted relief replay's effect (reliefreplay.fx): depth from the tint, alpha test.</summary>
        private Effect? _reliefReplayEffect;
        private SpriteBatch? _normalSpriteBatch;
        // Where the camera was when the sprite-normal buffer was last drawn. The buffer
        // is screen space, so a frame that cannot redraw it may only reuse it from the same view.
        // The field this describes lives in ScreenState now; see RenderPipeline.Screens.cs.
        ...
        // Whose light the meter is currently exposed for. A new one means arrive
        // already exposed rather than easing in from the last place's reading.
        // The field this describes lives in ScreenState now; see RenderPipeline.Screens.cs.

        // Eased exposure multiplier from the metering above. It scales the WHOLE frame,
        ...
        public RenderPipeline(GraphicsDevice device, IMonitor monitor, string modDirectory)
        {
            ...
            _reliefReplayEffect = LoadEffect("reliefreplay.mgfxo");
        ...
        internal Texture2D? LoadTexture(string file)
        {
            ...
            {
                _monitor.Log($"Failed to load {file}: {exception.Message}", LogLevel.Trace);
                return null;
            }
        }
        ...
        internal Texture2D? LoadTextureAt(string? fullPath)
        {
            ...
            {
                _monitor.Log($"Failed to load {fullPath}: {exception.Message}", LogLevel.Trace);
                return null;
            }
        }
        ...
        private Effect? LoadEffect(string file)
        {
            ...
            {
                _monitor.Log($"Failed to load {file} (that effect is disabled): {exception.Message}", LogLevel.Warn);
            }
            return null;
        }

        /// <summary>
        /// Reload one compiled shader from the assets folder without restarting the game: the
        /// author's tool for comparing two builds of a shader on ONE frozen frame, which is the
        /// only comparison that can call them byte-identical. Two launches differ by whatever the
        /// game itself animated between them, and measured on 12/9 that drift spread a hundred
        /// pixels and more through the effect chain, which is the whole screen.
        /// </summary>
        internal string ReloadShader(string name)
        {
            Effect? loaded = LoadEffect(name + ".mgfxo");
            if (loaded == null)
                return $"{name}.mgfxo did not load; the old one stays";
            Effect? old;
            switch (name)
            {
                case "floodlight": old = _floodEffect; _floodEffect = loaded; break;
                case "bloom": old = _bloom; _bloom = loaded; break;
                case "tiltshift": old = _tiltShift; _tiltShift = loaded; break;
                case "water": old = _water; _water = loaded; break;
                case "lighting": old = _lighting; _lighting = loaded; break;
                case "fog": old = _fogEffect; _fogEffect = loaded; break;
                case "cloudshadow": old = _cloudShadow; _cloudShadow = loaded; break;
                case "colorgrade": old = _colorGrade; _colorGrade = loaded; break;
                ...
                case "shadowmask": old = ShadowRenderer.ShadowMaskEffect; ShadowRenderer.ShadowMaskEffect = loaded; break;
                case "sheetscale": old = SheetUpscaler.Effect; SheetUpscaler.Effect = loaded; break;
                default: loaded.Dispose(); return $"no shader named {name}";
            }
            // The parameter cache is keyed by effect instance, so the old entries are only dead
            // weight; cleared so nothing can hand back a parameter of a disposed effect.
            _effectParameterCache.Clear();
            old?.Dispose();
            return $"{name} reloaded from assets";
        }

        private bool AnyEffectActive(ModConfig config) =>
            (config.FloodLightingEnabled && _floodEffect != null)
            || (config.LightingEnabled && _lighting != null)
            || (config.CloudShadowEnabled && _cloudShadow != null)
            || (config.BloomEnabled && _bloom != null && config.BloomIntensity > 0f)
            || ((config.FogEnabled || config.FogNightMist) && _fogEffect != null)
            || ((config.ColorGradeEnabled || config.BlueLightFilter > 0.001f) && _colorGrade != null)
            || (config.TiltShiftEnabled && _tiltShift != null)
            || ((config.WaterEnabled || config.WaterReflection) && _water != null)
            || config.WindowReflectionEnabled || _windowReflectEase > FadeGone
            || config.ParticlesEnabled || _fadeParticles > FadeGone
            || config.WetWorldEnabled || _fadeWet > FadeGone
            || ScreenEdgeDrops.WantedNow(config)
            || ((config.VignetteEnabled || config.ChromaticAberrationEnabled) && _finishing != null)
        ...
        internal string DescribeCameraKeyedCaches()
            => $"chainFrame={_frameWidth}x{_frameHeight} scaled={_lastScaledWidth}x{_lastScaledHeight}{(_scaledByZoom ? " (by the game zoom)" : "")} "
             ...
             + $"exposureSnaps={((uint)_activeScreenId < (uint)_exposureSnapsByScreen.Length ? _exposureSnapsByScreen[_activeScreenId] : 0)}";

        private void EnsureTargets(int width, int height, SurfaceFormat format)
        {
            width = Math.Max(1, width);
            height = Math.Max(1, height);

            if (_sceneRenderTarget != null && _sceneRenderTarget.Width == width && _sceneRenderTarget.Height == height && _sceneRenderTarget.Format == format)
                return;
            ...
            _sceneRenderTarget = CreateRenderTarget(width, height, format);
            _fullResolutionPingA = CreateRenderTarget(width, height, format);
            _fullResolutionPingB = CreateRenderTarget(width, height, format);
            _halfResolutionScratchA = CreateRenderTarget(Math.Max(1, width / 2), Math.Max(1, height / 2), format);
            _halfResolutionScratchB = CreateRenderTarget(Math.Max(1, width / 2), Math.Max(1, height / 2), format);
            _cloudMaskKeep = CreateRenderTarget(Math.Max(1, width / 2), Math.Max(1, height / 2), format);
            _cloudMaskTick = int.MinValue;   // whatever the new target holds, it is not a sky
        }

        /// <summary>The scale this frame was lowered by the game's zoom rather than by the player's
        /// setting (see ChooseRenderSize); the report says so beside the frame size.</summary>
        private bool _scaledByZoom;
        /// <summary>The config the last frame ran with, for the report's sharpness line: every
        /// setting that decides whether the world can look soft, in one line a reporter pastes.</summary>
        private ModConfig? _lastConfig;
        /// <summary>Which GI model actually built the lightmap last frame. The setting says
        /// cascades; a device that refuses RGBA16F targets runs the one-texel-per-tile flood
        /// map instead, and until now only the log said so.</summary>
        private bool _lastCascadesInUse;
        /// <summary>Below this zoom the chain follows the zoom down (the buffer is near four times
        /// the window at 50%); at and above it the chain runs at the buffer's size, because the
        /// round trip it saves costs the picture more than the pixels cost the card. Measured 5/9:
        /// see ChooseRenderSize.</summary>
        private const float ZoomCouplingBelow = 0.6f;
        ...
        /// <summary>Which world sprite won the depth test at each pixel of the relief buffer,
        /// as the sixteen bit rank the recorder handed the replay, high byte in red and low in
        /// green, with the sprite's coverage in alpha. Alpha is the only thing that says a
        /// sprite is there: the clear cannot give two bound targets different colours, so a
        /// pixel with no sprite carries the flat normal's bytes and a meaningless rank.
        /// <para>Written by the relief replay as its second output, so it costs that pass one
        /// more write and no draws of its own. It exists because a render target's DEPTH cannot
        /// be sampled on this build: it is a renderbuffer with no texture behind it.</para></summary>
        private RenderTarget2D? _spriteRankRenderTarget;
        private bool _spriteRankReady;

        /// <summary>radiance_spriterank: whether the relief replay writes the rank buffer at all.
        /// <para>OFF, because nothing reads it. The one thing that did - a lamp refusing to light
        /// what ranks in front of it - was built, measured and taken out again: draw order says
        /// what is drawn later, not what stands between the viewer and the lamp, and in a top-down
        /// room that meant a wall lamp lit nothing below itself. The buffer itself was never the
        /// thing that was wrong (proven at +0.932 against screen row, no measurable cost), so it
        /// stays here switched off rather than being deleted and rediscovered.</para>
        /// <para>Turn it on and dump a frame to see it: the dump carries it as sprite_rank.</para></summary>
        internal static bool SpriteRankWanted;

        /// <summary>The rank buffer, or null when this frame did not draw one.</summary>
        internal RenderTarget2D? SpriteRankBuffer => _spriteRankReady ? _spriteRankRenderTarget : null;

        private RenderTarget2D CreateRenderTarget(int width, int height, SurfaceFormat format) =>
            VramTally.Track(new RenderTarget2D(_device, width, height, false, format, DepthFormat.None, 0,
            ...
            ModConfig config)
        {
            ...
            // ARRIVING SOMEWHERE IS NOT A CHANGE IN THE LIGHT. The amounts below whose target
            // can be worked out on the spot - the shafts, the fog, the mist - are SET to it on the
            // frame the room changes rather than eased up from nothing. Walking out of the
            // farmhouse into a bright morning, the beams used to arrive over half a second, which
            // reads as the light being switched on rather than as light that was already there;
            // the game's own fade-to-black covers the step. The same reasoning the exposure meter
            // already follows, for the same complaint. The fades that wait on a mask being built
            // (water, cloud, lighting, the building shadow) keep easing in: their target is not
            // knowable until the thing exists, and snapping them would show a stage before its
            // buffer is ready.
            bool arrivedThisFrame = !ReferenceEquals(Game1.currentLocation, _fadeLocation);
            if (arrivedThisFrame)
            {
                ...
                SheetUpscaler.SoftSprites.AllowBurstThisTick();
            ...
            _lastCascadesInUse = cascadesWanted;
            ...
            bool floodOn = config.FloodLightingEnabled && _floodEffect != null
                && (!floodMapWanted || TimedBuild(config, 0, () => _flood.Build(_device, config)));
            bool classicOn = !floodOn && config.LightingEnabled && _lighting != null;
            if (floodOn)
            {
                long lightListStart = ChainStepBegin();
                BuildLightList(config);       // direct-light pools (shader term)
                ChainStepEnd(ChainStep.LightList, lightListStart);
                _isFloodOcclusionReady = TimedBuild(config, 1, () => BuildFloodOccluders(config));
            ...
            {
                long lightListStart = ChainStepBegin();
                classicOn = BuildLightList(config);
                ChainStepEnd(ChainStep.LightList, lightListStart);
            }
            if (classicOn)
                _shadowsReady = config.LightingShadows && TimedBuild(config, 2, () => BuildOccluderMask());
            ...
            if ((waterConfigOn && waterAllowedHere) || _fadeWater > FadeGone)
                TimedBuild(config, 3, () => BuildWaterMask());
            bool waterOn = waterConfigOn && _water != null && waterAllowedHere
                && Game1.currentLocation is { } waterLocation && LocationHasWater(waterLocation);
            ...
            long waterWatchStart = ChainStepBegin();
            ReportWaterWatch();
            ChainStepEnd(ChainStep.WaterWatch, waterWatchStart);
            bool waterStageRuns = _fadeWater > FadeGone && _water != null && _waterMask != null && _waterInMaskEase > FadeGone;
            // Map mist the world draw already skipped is owed to this frame even when the water
            // stage has just stopped wanting to run, so the stage stays for this one frame, at a
            // presence that is a plain copy once its fades are gone (see MistLayers).
            bool waterStageCarriesMist = !waterStageRuns && _water != null && _waterMask != null && MistLayers.WaitingOnThisScreen;
            if (waterStageRuns || waterStageCarriesMist) AddStage(_waterStageDelegate!, 2);
            ...
            // Mist on a map's always-front layer is air too, and goes the same way.
            PrecipitationSystem.DeferSkyDrawing(waterStageRuns);
            MistLayers.HoldOnThisScreen(waterStageRuns);
            ...
            bool wetOn = config.WetWorldEnabled && _wetEffect != null && outdoors
                && (WetnessNow > 0.004f || LocalSky.RainLandsHere);
            ...
            bool cloudOvercast = LocalSky.IsRaining || LocalSky.IsSnowing || LocalSky.IsLightning;
            Approach(ref _cloudWeatherAmount, cloudOvercast ? 0f : 1f, 0.05f);   // ~1s
            _cloudOvercastBlend = 1f - _cloudWeatherAmount;
            // Tomorrow's weather is already settled and already public: the television forecast
            // reads it out over breakfast. A sky that gives no sign of it is the one thing on
            // screen that does not know. So on the afternoon before rain the cover thickens and
            // the light loses a little of its warmth, arriving across the whole of the afternoon
            // rather than at an hour, and it is worth nothing on a day that stays clear.
            Approach(ref _stormWarningEased, StormWarningTarget(config), 0.05f);
            ...
            if (_fadeBuildingShadow > FadeGone && _cloudShadow != null) AddStage(_buildingShadowStageDelegate!, 4);
            ...
            {
                // Rain/snow: the overcast sky kills visible shafts outdoors, and under a glass
                // roof, which is the same sky.
                bool underSky = outdoors
                    || (config.GodRaysSunGlassRoof && ShadowRenderer.UnderAGlassRoof(Game1.currentLocation));
                bool overcast = underSky && (LocalSky.IsRaining || LocalSky.IsSnowing || LocalSky.IsLightning);
                ...
                if (underSky)
                {
                    int minutesNow = (Game1.timeOfDay / 100) * 60 + Game1.timeOfDay % 100;
                    rayDay = 1f - 0.7f * (1f - MathHelper.Clamp(Math.Abs(minutesNow - 750) / 270f, 0f, 1f));
                }
                float rayTarget = (config.GodRaysEnabled && config.FloodLightingEnabled && !overcast) ? rayDay : 0f;
                if (arrivedThisFrame) _godRayAmount = rayTarget;
                else Approach(ref _godRayAmount, rayTarget, 0.05f); // ~0.5s fade
            }
            // At an intensity of zero the composite returns the scene untouched, so the four
            // passes were fill for nothing; the stage is left out until there is something to add.
            if (config.BloomEnabled && _bloom != null && config.BloomIntensity > 0f) AddStage(_bloomStageDelegate!, 5);
            ...
            // torches out of the lightmap, so a lamp in the fog reads as a lamp in fog. The mine's
            // own fog is left alone underneath; this adds, it does not replace.
            // Two different thicknesses of air down there: the timed fog of an infested level
            // (isFogUp, which the game raises and clears on its own clock) and the standing haze
            // of the mushroom levels (ambientFog). Either counts.
            bool mineAirIsThick = Game1.currentLocation is StardewValley.Locations.MineShaft mineHere
                                  && (mineHere.isFogUp.Value || mineHere.ambientFog);
            MineAirIsThick = mineAirIsThick;
            float mistTarget = (config.FogNightMist && outdoors && !LocalSky.IsRaining && !LocalSky.IsSnowing)
                ? config.FogNightMistDensity * night
                : (mineAirIsThick ? config.MineFogMist : 0f);
            if (arrivedThisFrame) { _fogDayAmount = dayTarget; _fogMistAmount = mistTarget; }
            else
            {
            ...
            if ((_fogDayAmount > 0.004f || _fogMistAmount > 0.004f) && _fogEffect != null
                && (outdoors || mineAirIsThick)) AddStage(_fogStageDelegate!, 6);
            ...
            long breathScanStart = ChainStepBegin();
            ScanWaterBreathSources(config);
            ChainStepEnd(ChainStep.BreathScan, breathScanStart);
            ...
            // And not at a strength of zero, where the composite returns the sharp frame and the
            // two blur passes are fill for nothing; the fade below eases it out as it always did.
            bool tiltOn = config.TiltShiftEnabled && _tiltShift != null && !eventNow && config.TiltShiftStrength > 0f;
        ...
        public void Apply(SpriteBatch spriteBatch, ModConfig config)
        {
            ...
            StampEaseClock();
            // One slice of this frame for the whole-map walks, shared between all three of them.
            _mapScanBudget.BeginFrame();
            ...
            {
                if (config.DebugLogging) { _skippedNoTargetCount++; MaybeLogDiag(); }
                return;
            }

            int width = target.Width, height = target.Height;
            ChooseRenderSize(config, target, width, height, out int scaledWidth, out int scaledHeight,
            ...
            {
                _appliedFrameCount++;
                if (!_loggedOnce) { _monitor.Log($"Post-process {width}x{height}, format={target.Format}.", LogLevel.Debug); _loggedOnce = true; }
                MaybeLogDiag();
            }
            ...
            try
            {
                ...
                // Each step is timed on its own (RenderPipeline.ChainSteps.cs): the chain's one
                // cost row hid where a split screen was spending ten times a single one.
                long stepStart = ChainStepBegin();
                AdvanceWetness();

                var stages = BuildStageList(config);
                ChainStepEnd(ChainStep.StageList, stepStart);
                ...
                stepStart = ChainStepBegin();
                RenderNormalPass(config, target);
                ChainStepEnd(ChainStep.NormalPass, stepStart);
                ...
                // the emissive half.
                stepStart = ChainStepBegin();
                UpdateAndDrawAmbientParticles(spriteBatch, config, width);
                ChainStepEnd(ChainStep.Particles, stepStart);

                stepStart = ChainStepBegin();
                Texture2D chainSource = CaptureSceneForChain(spriteBatch, target, stages, scaledWidth, scaledHeight, scaled, ref captureTaken);
                ChainStepEnd(ChainStep.Capture, stepStart);
                ...
                if (config.ColorGradeEnabled && config.ColorGradeAuto)
                    ...
                    // AND BACK TO NEUTRAL WHEN THE METER IS SWITCHED OFF. The meter is the only
                    // writer of this number, so turning auto off used to leave whatever it was
                    // reading at that moment multiplying the whole picture for the rest of the
                    // session: stand somewhere bright, switch auto off, and the screen stays dim
                    // with no setting that explains it. It eases rather than snapping, because a
                    // whole-frame gain that jumps is the pop this family exists to avoid, and it
                    // eases every frame rather than every fourth, so the picture the player just
                    // asked for arrives within about a fifth of a second.
                    Approach(ref _meteredExposure, 1f, MeteredExposureEaseRate);
                ChainStepEnd(ChainStep.Exposure, stepStart);
                ...
                ChainStepEnd(ChainStep.Passes, stepStart);
                stepStart = ChainStepBegin();
                current = UpscaleToWindow(spriteBatch, current, target, stages.Count, scaled, config, width, height, renderScale);
                FillGainProbe(spriteBatch, target, stages.Count);
                ChainStepEnd(ChainStep.Upscale, stepStart);
                stepStart = ChainStepBegin();
                FinishFrame(spriteBatch, target, stages.Count, width, height, config);
                ...
                ReportMark(config);
                ChainStepEnd(ChainStep.Finish, stepStart);
            }
            catch (Exception exception)
            {
                RecoverFromChainFailure(spriteBatch, target, exception, captureTaken, width, height);
            }
            finally
            {
            ...
            if (config.DebugLogging)
            {
                ...
                double milliseconds = _performanceStopwatch.Elapsed.TotalMilliseconds;
                _performanceTotalMilliseconds += milliseconds;
                if (milliseconds > _performanceMaxMilliseconds) _performanceMaxMilliseconds = milliseconds;
            }
        }
        ...
        private void RecoverFromChainFailure(SpriteBatch spriteBatch, RenderTarget2D target,
                                             Exception exception, bool captureTaken, int width, int height)
        {
                _monitor.Log($"Post-process failed, leaving frame unmodified this frame: {exception.Message}", LogLevel.Warn);
                ...
                try
                {
                    ...
                    if (captureTaken && _sceneRenderTarget != null) spriteBatch.Draw(_sceneRenderTarget, new Rectangle(0, 0, width, height), Color.White);
                    spriteBatch.End();
                }
        ...
        /// </summary>
        private void ChooseRenderSize(ModConfig config, RenderTarget2D target, int width, int height,
                                      out int scaledWidth, out int scaledHeight, out bool scaled, out float renderScale)
        {
            _lastConfig = config;
                ...
                float chosenScale = renderScale;
                // "Costs nothing visible" above was measured on 5/9 and found wrong: the frame is
                // shrunk to the window size for the chain (a linear read), stretched back into the
                // buffer (another), and only then shrunk by the game to the window (a third). As the
                // window shows it, the world kept 66% of its edge contrast on the farm at 75% zoom
                // and 74% at 90%, against 100% at 100%, and six players reported the world soft
                // while the HUD stayed crisp. Writing back with a point read instead made it worse
                // (59% and 69%), so the round trip cannot be repaired, only avoided: the chain now
                // follows the zoom only below ZoomCouplingBelow, where the buffer is close to four
                // times the window and the frame-rate report that started this lives. At the zooms
                // people actually play at the chain runs at the buffer's size and the game's own
                // shrink is the only resample, as it is without this mod.
                if (zoomOut > 0.05f && zoomOut < ZoomCouplingBelow && !Game1.game1.takingMapScreenshot)
                    renderScale = Math.Max(0.25f, Math.Min(renderScale, zoomOut));
                _scaledByZoom = renderScale < chosenScale - 0.001f;
                scaledWidth = Math.Max(1, (int)Math.Round(width * renderScale));
                scaledHeight = Math.Max(1, (int)Math.Round(height * renderScale));
                scaled = scaledWidth != width || scaledHeight != height;
                _lastScaledWidth = scaledWidth; _lastScaledHeight = scaledHeight;
                _frameWidth = width; _frameHeight = height;
                EnsureTargets(scaledWidth, scaledHeight, target.Format);
        }
        ...
        private Texture2D CaptureSceneForChain(SpriteBatch spriteBatch, RenderTarget2D target, List<Action<SpriteBatch, Texture2D, RenderTarget2D, ModConfig>> stages, int scaledWidth, int scaledHeight, bool scaled, ref bool captureTaken)
        {
            ...
            // The master fade is stepped after the stages, and it blends this capture back: an
            // event that paints the screen starts it falling on a frame where it still reads 1.
            bool captureNeeded = scaled || stages.Count < 2 || _masterFade < 1f || FarmEventPaintsTheScreen
            ...
            if (captureNeeded)
            {
                ...
                spriteBatch.Draw(target, new Rectangle(0, 0, scaledWidth, scaledHeight), Color.White);
                spriteBatch.End();
                captureTaken = true;
            }
        ...
        private void RunBenchExtraChains(SpriteBatch spriteBatch, Texture2D chainSource, List<Action<SpriteBatch, Texture2D, RenderTarget2D, ModConfig>> stages, ModConfig config)
        {
            ...
            for (int repeat = 0; repeat < _benchExtraChains && stages.Count > 0; repeat++)
            {
                Texture2D scratch = chainSource;
                for (int i = 0; i < stages.Count; i++)
                {
                    RenderTarget2D destination = ReferenceEquals(scratch, _fullResolutionPingA) ? _fullResolutionPingB! : _fullResolutionPingA!;
                    stages[i](spriteBatch, scratch, destination, config);
                    scratch = destination;
                }
            }
        }
        ...
        private Texture2D RunStageChain(SpriteBatch spriteBatch, Texture2D chainSource, RenderTarget2D target, List<Action<SpriteBatch, Texture2D, RenderTarget2D, ModConfig>> stages, bool scaled, ModConfig config)
        {
            ...
            for (int i = 0; i < stages.Count; i++)
            {
                ...
                RenderTarget2D destination = i == stages.Count - 1 && !scaled
                    ? target
                ...
                int stageNameIndex = _stageNameIndices[i];
                BeginStageCost(stageNameIndex);
                stages[i](spriteBatch, current, destination, config);
                double milliseconds = EndStageCost();
                if (_timingOn)
                {
                    _stageMilliseconds[stageNameIndex] += milliseconds;
                    if (milliseconds > _stageMaxMilliseconds[stageNameIndex]) _stageMaxMilliseconds[stageNameIndex] = milliseconds;
                    _stageRunFrames[stageNameIndex]++;
                }
                current = destination;
            }
        ...
        private Texture2D UpscaleToWindow(SpriteBatch spriteBatch, Texture2D current, RenderTarget2D target, int stageCount, bool scaled, ModConfig config, int width, int height, float renderScale)
        {
            ...
            if (scaled && stageCount > 0)
            {
                if (_upscale != null)
                {
                    GetParam(_upscale, "OutputTexel")?.SetValue(new Vector2(1f / width, 1f / height));
                ...
                else
                {
                    _device.SetRenderTarget(target);
                    spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.Opaque, SamplerState.LinearClamp);
                    spriteBatch.Draw(current, new Rectangle(0, 0, width, height), Color.White);
                    spriteBatch.End();
                }
                current = target;
            }
        ...
        private void FinishFrame(SpriteBatch spriteBatch, RenderTarget2D target, int stageCount, int width, int height, ModConfig config)
        {
            ...
            _masterFade = FarmEventPaintsTheScreen
                ? RampTo0(_masterFade, MasterFadeOutUnderFarmEvent)
                : RampTo1(_masterFade, MasterFadeInPerFrame);
            if (_masterFade < 1f && stageCount > 0)
            {
                _device.SetRenderTarget(target);
                spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp);
                spriteBatch.Draw(_sceneRenderTarget, new Rectangle(0, 0, width, height), Color.White * (1f - _masterFade));
                spriteBatch.End();
            }
            ...
            if (GpuProbe)
            {
                double gpuMilliseconds = ProbeGpuTime(spriteBatch, target);
                if (BenchRunning)
                    BenchTick(config, gpuMilliseconds);
                else if (EffectCostRunning)
                    EffectCostTick(config, gpuMilliseconds);
            }
            ...
            {
                try { WriteDump(spriteBatch, target, width, height, config); }
                catch (Exception exception) { _monitor.Log($"radiance_dump failed: {exception.Message}", LogLevel.Warn); }
            ...
            if (_burstFramesWanted > 0)
            {
                ...
                {
                    _monitor.Log($"radiance_dumpburst failed: {exception.Message}", LogLevel.Warn);
        ...
        private void RedrawEventSkipButton(SpriteBatch spriteBatch, RenderTarget2D target)
        {
            ...
            if (currentEvent == null || !currentEvent.skippable
        ...
        /// <summary>0 .. 1: how far the sky has closed in ahead of tomorrow's rain. Static because
        /// the colour grade's automatic mood is a static method and reads the same number the cloud
        /// stage does; there is one pipeline and one sky.</summary>
        private static float _stormWarningEased;
        /// <summary>Whether the mine level on screen has its fog up, for the report. A mist that
        /// does not appear and a level that was never foggy are the same picture.</summary>
        internal static bool MineAirIsThick;
        /// <summary>The middle of the close-in, and how long it takes. Centred at half past five
        /// with three hours of width, so it begins around four and is fully arrived by seven, and
        /// no single hour carries a visible change.</summary>
        private const int StormWarningCentreHhmm = 1730;
        private const float StormWarningHalfWidthMinutes = 90f;
        /// <summary>Vanilla's weather ids for tomorrow. There are no constants for these in 1.6:
        /// weather moved into Data/LocationContexts and the ids are plain strings, so the ones
        /// vanilla ships are written out here. An id from a weather mod that is none of these is
        /// simply not a storm as far as this is concerned, which is the safe way to be wrong.</summary>
        private static readonly string[] StormWarningWeatherIds = ["Rain", "Storm", "GreenRain", "Snow"];
        /// <summary>How much more of the ground is under cloud at a full close-in. A fifth on top of
        /// the player's own coverage, which is well short of the 0.32 a real overcast adds.</summary>
        private const float StormWarningExtraCoverage = 0.20f;

        /// <summary>How closed in the sky should be right now, before easing: the dial, held back
        /// until the afternoon, and only on a day whose tomorrow brings weather.</summary>
        private static float StormWarningTarget(ModConfig config)
        {
            if (config.StormWarningStrength <= 0.001f)
                return 0f;
            string tomorrow = Game1.weatherForTomorrow;
            if (string.IsNullOrEmpty(tomorrow))
                return 0f;
            bool weatherIsComing = false;
            foreach (string id in StormWarningWeatherIds)
                if (string.Equals(tomorrow, id, StringComparison.OrdinalIgnoreCase)) { weatherIsComing = true; break; }
            if (!weatherIsComing)
                return 0f;
            return config.StormWarningStrength * GameClock.RampAt(StormWarningCentreHhmm, StormWarningHalfWidthMinutes);
        }
        // The cloud stage's blurred density mask, kept in its OWN target so it survives
        ...
        // Cross-frame RT — PreserveContents (CreateRenderTarget default) is load-bearing.
        // The field this describes lives in ScreenState now; see RenderPipeline.Screens.cs.
        // Tick _cloudMaskKeep was last written; the flood stage refuses a
        // stale mask (cloud stage off, or just faded out) rather than gating on old sky.
        // The field this describes lives in ScreenState now; see RenderPipeline.Screens.cs.
        // Viewport origin in world tiles when the kept mask was drawn, so the flood
        // stage can shift its sample by however far the camera moved since.
        // The field this describes lives in ScreenState now; see RenderPipeline.Screens.cs.
        // The opacity the kept mask was composited at (already carries day factor and
        // fade), so the shafts respect a faint moon-cloud faintly and a storm ceiling hard.
        // The field this describes lives in ScreenState now; see RenderPipeline.Screens.cs.
        // Eased coupling strength, so shafts do not pop when the cloud stage joins or
        // leaves the frame — same house rule as every other gate on this effect.
        // The field this describes lives in ScreenState now; see RenderPipeline.Screens.cs.
        ...
        // _wadingEase lives in ScreenState: it follows whether this screen's player stands in water.

        private readonly EffectParamCache _effectParameterCache = new();

        private EffectParameter? GetParam(Effect effect, string name) => _effectParameterCache.Get(effect, name);

        /// <summary>Draw one full-screen pass of <paramref name="effect"/> from
        /// <paramref name="source"/> into <paramref name="destination"/>. This was written twice,
        /// as Pass and as DrawFull, byte for byte the same; two names for one thing is two places
        /// for a future change to land in and one of them to be missed.</summary>
        private void Pass(SpriteBatch spriteBatch, Texture2D source, RenderTarget2D destination, Effect effect)
        ...
        private void PassAdd(SpriteBatch spriteBatch, Texture2D source, RenderTarget2D destination, Effect effect)
        {
        ...
        private void DrawFull(SpriteBatch spriteBatch, Texture2D source, RenderTarget2D destination, Effect effect)
        {
            _device.SetRenderTarget(destination);
        ...
        private void BlendBackSource(SpriteBatch spriteBatch, Texture2D source, RenderTarget2D destination, float presence)
        {
            if (presence >= 0.999f)
                return;
            _device.SetRenderTarget(destination);
        ...
        private readonly Dictionary<int, BlendState> _lerpBlends = [];
        ...
        private BlendState LerpBlend(float weight)
        {
            int quantised = MathHelper.Clamp((int)Math.Round(weight * 255f), 0, 255);
            if (!_lerpBlends.TryGetValue(quantised, out BlendState? blendState))
            {
                float factor = quantised / 255f;
                blendState = new BlendState
                {
                    ...
                    BlendFactor = new Color(factor, factor, factor, factor),
                };
                _lerpBlends[quantised] = blendState;
            }
            return blendState;
        }
        ...
        public void Dispose()
        {
            ...
            _wetSuitabilityTexture?.Dispose(); _wetSuitabilityTexture = null;
            _wetSuitabilitySpare?.Dispose(); _wetSuitabilitySpare = null;
            _wetSuitabilityUploaded = null;
            foreach (var kept in _surfaceClassTextures.Values)
                kept.Texture?.Dispose();
            _surfaceClassTextures.Clear();
            ...
            _water?.Dispose(); _finishing?.Dispose(); _lighting?.Dispose(); _floodEffect?.Dispose(); _flood.Dispose(); _cascadesEffect?.Dispose(); _cascades.Dispose(); _normalsEffect?.Dispose(); _sheetNormals.Dispose(); _normalRenderTarget?.Dispose(); _normalRenderTarget = null; _flatNormalTexture?.Dispose(); _flatNormalTexture = null; _reliefReplayEffect?.Dispose(); _reliefReplayEffect = null; _reliefVertexPath?.Dispose(); _reliefVertexPath = null; SheetUpscaler.Dispose(); SpriteFlushPath.Dispose(); _normalSpriteBatch?.Dispose(); _normalSpriteBatch = null;
            _heatMapTexture?.Dispose();
            _heatMapSpare?.Dispose(); _heatMapSpare = null;
            ...
            _effectParameterCache.Clear(); // parameter cache keys pin the disposed Effects otherwise
