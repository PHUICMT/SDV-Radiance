...
namespace SDVRadiance
{
    ...
    internal sealed partial class ShadowRenderer
    {
        internal static string Report(ModConfig config, bool wholeMap)
        {
            ...
            sb.AppendLine($"[shadows] location={loc.NameOrUniqueName} outdoors={loc.IsOutdoors} time={Game1.timeOfDay} season={Game1.season}");
            sb.AppendLine($"[shadows] path={(SunCasts() ? "SUN" : "PER-LIGHT")} shouldCast={ShouldCast(config)} strength={config.DirectionalShadowStrength:0.00} objectsEnabled={config.DirectionalShadowObjects}");
            ...
            sb.AppendLine($"[shadows] eventUp={Game1.eventUp} currentEvent={(ev != null)} isFestival={ev?.isFestival} "
                        + $"showWorldCharacters={ev?.showWorldCharacters} showGroundObjects={loc.currentEvent?.showGroundObjects} "
                        + $"actors={ev?.actors?.Count ?? 0} residents={loc.characters.Count}");

            var vp = Game1.viewport;
            int vx0 = vp.X / 64 - 1, vx1 = (vp.X + vp.Width) / 64 + 1;
            int vy0 = vp.Y / 64 - 1, vy1 = (vp.Y + vp.Height) / 64 + 1;
            ...
            const int Pad = 20;
            int tx0 = wholeMap ? 0 : vx0 - Pad, tx1 = wholeMap ? loc.Map?.Layers[0]?.LayerWidth ?? vx1 : vx1 + Pad;
            int ty0 = wholeMap ? 0 : vy0 - Pad, ty1 = wholeMap ? loc.Map?.Layers[0]?.LayerHeight ?? vy1 : vy1 + Pad;
            bool OnScreen(int x, int y) => x >= tx0 && x <= tx1 && y >= ty0 && y <= ty1;
            sb.AppendLine($"[shadows] scanning tiles {tx0},{ty0} to {tx1},{ty1} ({(wholeMap ? "whole map" : $"screen +{Pad} tiles")}); "
            ...
            {
                Vector2 p = Game1.GlobalToLocal(vp, new Vector2(wx, wy));
                return $"screen={(int)p.X},{(int)p.Y}";
            }
            string Mark(int x, int y) => Visible(x, y) ? " " : "*";
            Vector2 cur = Game1.currentCursorTile;
            sb.AppendLine($"[shadows] cursor tile={cur.X},{cur.Y} player tile={Game1.player?.TilePoint} viewport={vp.X},{vp.Y}");

            sb.AppendLine("[shadows] characters (verdict = what the shadow pass does with it):");
            int shown = 0;
            foreach (NPC npc in CharactersIn(loc))
            {
                ...
                string verdict =
                    ...
                    : OnOpenWater(loc, t) ? "SKIP on open water"
                ...
                Vector2 anchorPt = Game1.GlobalToLocal(vp, new Vector2(
                    npc.Position.X + npc.GetSpriteWidthForPositioning() * 4 / 2f, npc.GetBoundingBox().Bottom));
                Vector2 gameAnchor = npc.getLocalPosition(vp)
                ...
                sb.AppendLine($"{Mark(t.X, t.Y)} {npc.Name,-16} tile={t.X},{t.Y} anchorY={(int)anchorPt.Y} "
                            ...
                            + $"water={OnWater(loc, t)} "
                            + $"openWater={OnOpenWater(loc, t)} -> {verdict}");
            }
            if (shown == 0)
                sb.AppendLine("  (none — if you can see NPCs, they are not in the list the pass reads)");
            ...
            sb.AppendLine($"[shadows] object gates: objectsDrawn={(!eventUp || showGround)} "
                        + $"furnitureDrawn={(!eventUp || loc is Farm || loc is StardewValley.Locations.FarmHouse)} "
                        + $"clumpsDrawn={!(loc is StardewValley.Locations.Woods && eventUp && !showGround)}");
            sb.AppendLine("[shadows] objects, furniture, clumps, plants, animals and critters:");
            int objs = 0;
            foreach (var kv in loc.objects.Pairs)
            {
                ...
                sb.AppendLine($"{Mark((int)tile.X, (int)tile.Y)} {o.Name,-20} tile={tile.X},{tile.Y} {Screen(tile.X * 64 + 32, tile.Y * 64 + 64)} {kind} passable={o.isPassable()} "
            ...
            foreach (Furniture f in loc.furniture)
            {
                if (f == null || !OnScreen((int)f.TileLocation.X, (int)f.TileLocation.Y))
                    continue;
                objs++;
                sb.AppendLine($"{Mark((int)f.TileLocation.X, (int)f.TileLocation.Y)} {f.Name,-20} tile={f.TileLocation.X},{f.TileLocation.Y} furniture type={f.furniture_type.Value}");
            }
            foreach (ResourceClump c in loc.resourceClumps)
            {
                if (c == null || !OnScreen((int)c.Tile.X, (int)c.Tile.Y))
                    continue;
                objs++;
                sb.AppendLine($"{Mark((int)c.Tile.X, (int)c.Tile.Y)} clump {c.parentSheetIndex.Value,-14} tile={c.Tile.X},{c.Tile.Y}");
            }
            foreach (var ltf in loc.largeTerrainFeatures)
            {
                if (ltf == null || !OnScreen((int)ltf.Tile.X, (int)ltf.Tile.Y))
                    continue;
                objs++;
                sb.AppendLine($"{Mark((int)ltf.Tile.X, (int)ltf.Tile.Y)} {ltf.GetType().Name,-20} tile={ltf.Tile.X},{ltf.Tile.Y} largeTerrainFeature");
            }
            // terrainFeatures is tile-keyed and can be huge, so it is walked by the visible window.
            for (int y = ty0; y <= ty1; y++)
            for (int x = tx0; x <= tx1; x++)
            {
                if (!loc.terrainFeatures.TryGetValue(new Vector2(x, y), out var tf) || tf is Flooring)
                    continue;
                objs++;
                ...
                sb.AppendLine($"{Mark(x, y)} {tf.GetType().Name,-20} tile={x},{y} {extra}");
            }
            // Critters and animals cast too, and neither is in loc.objects. A beach in winter has
            ...
            foreach (FarmAnimal a in AnimalsIn(loc))
            {
                if (a == null || !OnScreen(a.TilePoint.X, a.TilePoint.Y))
                    continue;
                objs++;
                sb.AppendLine($"{Mark(a.TilePoint.X, a.TilePoint.Y)} {a.Name,-20} tile={a.TilePoint.X},{a.TilePoint.Y} {Screen(a.Position.X, a.GetBoundingBox().Bottom)} animal");
            }
            if (loc.critters != null)
                foreach (var c in loc.critters)
                {
                    if (c == null)
                        continue;
                    objs++;
                    sb.AppendLine($"{Mark((int)(c.position.X / 64), (int)(c.position.Y / 64))} {c.GetType().Name,-20} tile={(int)(c.position.X / 64)},{(int)(c.position.Y / 64)} "
                                + $"{Screen(c.position.X, c.position.Y)} critter");
                }
            if (objs == 0)
                sb.AppendLine("  (none — an orphan shadow here comes from something not in these lists)");
